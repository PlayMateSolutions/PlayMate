...
function recordPayment(paymentData) {
  ...
  const newRow = [
    ...
    paymentData.amount,
    paymentData.paymentType || PAYMENT_TYPES[0],
  ...
  paymentsSheet.getRange(lastRow, PAYMENTS_COLUMNS.AMOUNT + 1).setNumberFormat('$0.00');
  
  if (paymentData.periodStart) {
    paymentsSheet.getRange(lastRow, PAYMENTS_COLUMNS.PERIOD_START + 1).setNumberFormat('yyyy-mm-dd');
  }
...
function updatePayment(paymentId, updatedData) {
  ...
  if (updatedData.memberId !== undefined) {
    paymentsSheet.getRange(rowIndex, PAYMENTS_COLUMNS.MEMBER_ID + 1).setValue(updatedData.memberId);
  }
  
  if (updatedData.date !== undefined) {
    paymentsSheet.getRange(rowIndex, PAYMENTS_COLUMNS.DATE + 1).setValue(updatedData.date);
  }
  
  if (updatedData.amount !== undefined) {
    paymentsSheet.getRange(rowIndex, PAYMENTS_COLUMNS.AMOUNT + 1).setValue(updatedData.amount);
  }
  
  if (updatedData.paymentType !== undefined) {
    paymentsSheet.getRange(rowIndex, PAYMENTS_COLUMNS.PAYMENT_TYPE + 1).setValue(updatedData.paymentType);
  }
  
  if (updatedData.periodStart !== undefined) {
    paymentsSheet.getRange(rowIndex, PAYMENTS_COLUMNS.PERIOD_START + 1).setValue(updatedData.periodStart);
  }
  
  if (updatedData.periodEnd !== undefined) {
    paymentsSheet.getRange(rowIndex, PAYMENTS_COLUMNS.PERIOD_END + 1).setValue(updatedData.periodEnd);
  }
  
  if (updatedData.status !== undefined) {
    paymentsSheet.getRange(rowIndex, PAYMENTS_COLUMNS.STATUS + 1).setValue(updatedData.status);
  }
  
  if (updatedData.notes !== undefined) {
    paymentsSheet.getRange(rowIndex, PAYMENTS_COLUMNS.NOTES + 1).setValue(updatedData.notes);
  }
...
function getPaymentRecords(filters = {}) {
  ...
  for (let i = 1; i < paymentsData.length; i++) {
    ...
    // Apply filters
    if (filters.memberId && row[PAYMENTS_COLUMNS.MEMBER_ID] !== filters.memberId) {
      continue;
    }
...
function getOverallPaymentSummary(options = {}) {
  ...
  for (const record of paymentRecords) {
    ...
    const monthStr = Utilities.formatDate(new Date(record.date), Session.getScriptTimeZone(), 'yyyy-MM');
...
function getMembersWithPaymentStatus(sport = null) {
  ...
  for (const member of members) {
    ...
    for (const sportName of sportsToCheck) {
      ...
      paymentsWithEndDate.sort((a, b) => new Date(b.periodEnd) - new Date(a.periodEnd));
